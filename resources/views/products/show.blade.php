...
<div class="min-h-screen pb-16" style="background:#F8FCF7;"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        <nav class="flex items-center gap-2 text-sm mb-6" style="color:#5C6352;">
            <a href="{{ route('products.index') }}" style="color:#5C6352;text-decoration:none;" class="hover:underline">{{ __('products.page_title') }}</a>
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0 space-y-5">
                    ...
                    <div class="relative product-gallery-main" style="background:#EDF0E5;">
                        ...
                        @if($product->isSold())
                            <div class="absolute top-4 left-4 px-4 py-1.5 rounded-full text-sm font-bold"
                                 style="background:rgba(0,0,0,.7);color:white;">{{ __('products.sold_badge') }}</div>
                        @endif
                    </div>
                ...
                <div class="bg-white rounded-2xl p-6" style="border:1px solid #EDF0E5;">
                    <h2 class="font-serif text-xl font-bold text-ink mb-4">{{ __('products.description_title') }}</h2>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-5 pt-5" style="border-top:1px solid #EDF0E5;">
                        @if($product->category?->parent?->name || $product->category?->name)
                            <div class="attr-row">
                                <span class="attr-label">{{ __('products.breed') }}</span>
                                <span class="attr-value">{{ $product->category?->name }}</span>
                            </div>
                        @endif
                        @if($product->age)
                            <div class="attr-row">
                                <span class="attr-label">{{ __('products.age_label') }}</span>
                                <span class="attr-value">{{ $product->age }} {{ __('products.age_unit') }}</span>
                            </div>
                        @endif
                        @if($product->weight)
                            <div class="attr-row">
                                <span class="attr-label">{{ __('products.weight_label') }}</span>
                                <span class="attr-value">{{ $product->weight }} kg</span>
                            </div>
                        @endif
                        @if($product->gender)
                            <div class="attr-row">
                                <span class="attr-label">{{ __('products.gender_label') }}</span>
                                <span class="attr-value">{{ $product->gender === 'erkak' ? __('products.gender_male') : __('products.gender_female') }}</span>
                            </div>
                        @endif
                        @if($product->color)
                            <div class="attr-row">
                                <span class="attr-label">{{ __('products.color_label') }}</span>
                                <span class="attr-value">{{ $product->color->name }}</span>
                            </div>
                        @endif
                        @if($product->status)
                            <div class="attr-row">
                                <span class="attr-label">{{ __('products.health_label') }}</span>
                                <span class="attr-value">{{ __('products.health_value') }}</span>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl overflow-hidden" style="border:1px solid #EDF0E5;">
                    <div class="px-6 pt-5 pb-3 flex items-center justify-between">
                        <h2 class="font-serif text-xl font-bold text-ink">{{ __('products.location_title') }}</h2>
            ...
            <div class="w-full lg:w-[340px] flex-shrink-0">
                <div class="sidebar-sticky space-y-4">
                    ...
                    <div class="bg-white rounded-2xl p-5" style="border:1px solid #EDF0E5;">
                        ...
                        <div class="flex flex-wrap gap-2 mb-3">
                            ...
                            @if($product->gender)
                                <span style="background:#EDF0E5;color:#5C6352;font-size:.75rem;font-weight:700;padding:4px 10px;border-radius:6px;">
                                    {{ $product->gender === 'erkak' ? __('products.gender_male') : __('products.gender_female') }}
                                </span>
                            @endif
                            @if($product->status)
                        ...
                        <p class="text-xs flex items-center gap-3 mb-5" style="color:#5C6352;">
                            <span class="flex items-center gap-1">
                                ...
                                {{ number_format($product->views_count) }} {{ __('products.views_display') }}
                            </span>
                        ...
                        @if(!$product->isSold())
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <template x-if="showPhone">
                                            <div class="contact-reveal">
                                                <a :href="'tel:' + phone" @click="trackCall()"
                                                   ...
                                                   x-text="phone"></a>
                                                <p class="text-center text-xs mt-1.5" style="color:#5C6352;">{{ __('products.call_hint') }}</p>
                                            </div>
                                        </template>
                                        ...
                                        <div class="flex gap-2">
                                            <button @click="toggleFav()" :disabled="loadingFav"
                                                ...
                                                <span x-text="favorited ? '{{ __('products.saved_btn') }}' : '{{ __('products.save_btn') }}'"></span>
                                            </button>
                            ...
                            @else
                                ...
                                <div class="rounded-xl p-4 text-center mb-2" style="background:#EDF0E5;">
                                    <p class="text-sm font-semibold text-ink mb-3">{{ __('products.login_to_see_phone') }}</p>
                                    <div class="flex gap-2 justify-center">
                                        <a href="{{ route('login') }}"
                                           style="padding:9px 20px;background:#1D3520;color:white;border-radius:8px;font-weight:700;font-size:.85rem;text-decoration:none;">{{ __('products.login_btn') }}</a>
                                        <a href="{{ route('register') }}"
                                           style="padding:9px 20px;background:white;color:#1D3520;border-radius:8px;font-weight:700;font-size:.85rem;text-decoration:none;border:1.5px solid #E2ECDF;">{{ __('products.register_btn') }}</a>
                                    </div>
                                </div>
                            @endauth
                        @else
                            <div class="rounded-xl p-5 text-center mb-2" style="background:#f0f0f0;">
                                ...
                                <p class="font-semibold text-ink">{{ __('products.product_sold') }}</p>
                            </div>
                        @endif
                    </div>
                    ...
                    {{-- Seller card --}}
                    <div class="bg-white rounded-2xl p-5" style="border:1px solid #EDF0E5;">
                        <p class="text-xs font-semibold uppercase tracking-widest mb-4" style="color:#5C6352;">{{ __('products.seller') }}</p>
                        ...
                        <a href="{{ route('seller.show', $product->user) }}" style="text-decoration:none;display:flex;align-items:center;gap:12px;margin-bottom:14px;">
                            ...
                            <div>
                                ...
                                <p class="text-xs flex items-center gap-1" style="color:#3E683F;">
                                    ...
                                    {{ __('products.verified_farmer') }} · {{ $product->user->created_at?->year }}
                                </p>
                            </div>
                        </a>
                        ...
                        @if($sellerStats)
                            <div class="grid grid-cols-3 gap-2 mb-4 text-center">
                                <div class="rounded-xl py-2.5" style="background:#F8FCF7;">
                                    ...
                                    <p class="text-xs" style="color:#5C6352;">{{ __('products.active_ads') }}</p>
                                </div>
                                <div class="rounded-xl py-2.5" style="background:#F8FCF7;">
                                    <p class="font-bold text-base text-ink">{{ $sellerStats['total'] }}</p>
                                    <p class="text-xs" style="color:#5C6352;">{{ __('products.total_ads') }}</p>
                                </div>
                                <div class="rounded-xl py-2.5" style="background:#F8FCF7;">
                                    <p class="font-bold text-base text-ink">{{ $sellerStats['sold'] }}</p>
                                    <p class="text-xs" style="color:#5C6352;">{{ __('products.sold_count') }}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="flex items-center gap-3 py-2" style="color:#5C6352;">
                            ...
                            <p class="text-sm">{{ __('products.no_seller') }}</p>
                        </div>
                        @endif
                    </div>
                    ...
                    <div class="rounded-2xl p-4 flex gap-3" style="background:#F6ECD7;border:1px solid #e8d5a8;">
                        ...
                        <div>
                            <p class="font-bold text-sm mb-1" style="color:#B5822A;">{{ __('products.safety_title') }}</p>
                            <p class="text-xs leading-relaxed" style="color:#9a6d22;">{{ __('products.safety_desc') }}</p>
                        </div>
                    </div>
        ...
        @if($relatedProducts->isNotEmpty())
            <div class="mt-12">
                <h2 class="font-serif text-2xl font-bold text-ink mb-6">{{ __('products.related_ads') }}</h2>
                        ...
                        <a href="{{ route('products.show', $rel) }}" class="related-card">
                            <div style="height:160px;background:#EDF0E5;overflow:hidden;position:relative;">
                                ...
                                @if($rel->created_at->diffInDays() < 3)
                                    <span style="position:absolute;top:10px;left:10px;background:#B5822A;color:white;font-size:.62rem;font-weight:700;padding:3px 8px;border-radius:5px;text-transform:uppercase;">{{ __('products.new_tag') }}</span>
                                @endif
                            </div>
                            <div class="p-4">
                                ...
                                <div class="flex gap-2 text-xs" style="color:#5C6352;">
                                    @if($rel->age)<span>{{ $rel->age }} {{ __('products.age_unit') }}</span>@endif
            ...
            <div x-data="{ open: false }" @open-sold-modal.window="open = true">
                <div x-show="open" x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
                        <h3 class="font-serif text-lg font-bold text-ink mb-5">{{ __('products.sold_modal_title') }}</h3>
                        <form method="POST" action="{{ route('products.mark-sold', $product) }}" class="space-y-4">
                            @csrf
                            <div>
                                <label class="block text-xs font-semibold uppercase tracking-wide mb-1.5" style="color:#5C6352;">{{ __('products.sold_price_modal') }}</label>
                            ...
                            <div>
                                <label class="block text-xs font-semibold uppercase tracking-wide mb-1.5" style="color:#5C6352;">{{ __('products.sale_source_label') }}</label>
                                <select name="source"
                                    ...
                                    <option value="platform_chat">{{ __('products.source_platform') }}</option>
                                </select>
                            </div>
